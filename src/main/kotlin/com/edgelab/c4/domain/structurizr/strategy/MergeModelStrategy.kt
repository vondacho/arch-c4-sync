...
import com.edgelab.c4.domain.C4Tags.purge
...
class MergeModelStrategy(
    ...
    fun execute(
        ...
        val enterprise = { model: Model ->
            model.enterprise = Enterprise(
                "model.enterprise.name".electValue(
                    master.enterprise?.name?.emptyIf(JOCKER),
                    scores,
                    other.enterprise?.name?.emptyIf(JOCKER),
                    weights
                )
            )
        }
    ...
    private fun Person.merge(other: Person, scores: PropertyScores, weights: PropertyWeights): Promotion<Model> =
        { model ->
            val person = model.addPerson(
                "person.location".electValue(this.location, scores, other.location, weights),
    ...
    private fun Person.add(scores: PropertyScores, weights: PropertyWeights): Promotion<Model> =
        { model ->
            val person = model.addPerson(
                "person.location".takeValue(this.location, scores, weights),
    ...
    private fun SoftwareSystem.merge(
        ...
        { model ->
            val system = model.addSoftwareSystem(
                "system.location".electValue(this.location, scores, other.location, weights),
    ...
    private fun SoftwareSystem.add(scores: PropertyScores, weights: PropertyWeights): Promotion<Model> =
        { model ->
            val system = model.addSoftwareSystem(
                "system.location".takeValue(this.location, scores, weights),
        ...
        { container ->
            val component = container.addComponent(
                "component.name".electValue(this.name?.notEmpty(), scores, other.name?.notEmpty(), weights)!!
                    .canonize(nameCanonizer),
                "component.type".electValue(this.type, scores, other.type, weights)?.type ?: "",
            ...
            "component.code".electValue(this.code, scores, other.code, weights)!!
                .onEach { component.addSupportingType(it.type) }
        }
    ...
    private fun Component.add(scores: PropertyScores, weights: PropertyWeights): Promotion<Container> =
        { container ->
            val component = container.addComponent(
                ...
                "component.type".takeValue(this.type, scores, weights)?.type ?: "",
            ...
            "component.properties".takeValue(this.properties, scores, weights)
                ?.onEach { component.addProperty(it.key, it.value) }
            "component.code".takeValue(this.code, scores, weights)!!
                .onEach { component.addSupportingType(it.type) }
        }
    ...
    private fun DeploymentNode.merge(
        ...
        { model ->
            val node = model.addDeploymentNode(
                ...
                "node.instances".electValue(this.instances, scores, other.instances, weights) ?: 1,
    ...
    private fun DeploymentNode.add(scores: PropertyScores, weights: PropertyWeights): Promotion<Model> =
        { model ->
            val node = model.addDeploymentNode(
                ...
                "node.instances".takeValue(this.instances, scores, weights) ?: 1,
